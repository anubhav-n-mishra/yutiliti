...
export async function POST(req: Request) {
  try {
    ...
    if (!apiKey) {
      ...
      let reply = `Yuitility is a 100% private in-browser toolkit with 131 free tools. How can I help with your math, PDF, image, health, or developer calculations?`;
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({
        reply: "I am ready to assist with Yuitility's 131 free browser tools. For enterprise custom web/mobile app engineering, visit [Amvelt.com](https://amvelt.com).",
      });
    }

    const data = await response.json();
    const reply = data.choices?.[0]?.message?.content || "How can I assist with Yuitility's 131 browser tools?";

    return NextResponse.json({ reply });
  } catch (error) {
    console.error("[AI Route Error]:", error);
    return NextResponse.json({
      reply: "Yuitility offers 131 browser tools running 100% in local memory. For custom web development or SEO, visit [Amvelt.com](https://amvelt.com).",
    });
  }
}
